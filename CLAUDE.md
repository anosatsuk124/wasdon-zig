...
- **Translator-time metadata via `__udon_meta`.** Per `docs/spec_udonmeta_conversion.md`, the translator may consume a static UTF-8 JSON blob named `__udon_meta` describing how globals/symbols become data-section fields, how exported functions map to Udon event labels (`_start`, `_update`, `_interact`, custom), sync modes, and translator options. The blob lives in a sidecar file (never inside the Wasm binary), and if absent the translator falls back to defaults — it is **not** read at runtime.
- **`__udon_meta` discovery contract.** `__udon_meta` is a sidecar JSON file passed alongside the `.wasm` input. The CLI accepts it via `--meta <path.json>`; if `--meta` is omitted, it auto-discovers `<wasm-stem>.udon_meta.json` next to the `.wasm` (e.g. `bench.wasm` → `bench.udon_meta.json`). When passing JSON bytes through the library, use the `udon_meta_json: ?[]const u8` parameter on `wasdon_zig.translateBytes`; pass `null` for "no meta". Producers (Zig, Rust, hand-rolled WAT) need not embed anything related to `__udon_meta` in the binary — no `__udon_meta_ptr` / `__udon_meta_len` exports, no data segments. The bytes are decoded by `wasm.parseUdonMeta` (`src/wasm/udon_meta.zig`'s `parse`).
...
- Reproducible producer-side pitfall (Rust/Zig/WAT) discovered → append it to the producer-facing guide `docs/producer_guide.md` (covers toolchain pinning, `no_std` skeleton, extern declarations, the `__udon_meta` sidecar JSON contract, recursion opt-in, memory sizing, and the build → translate pipeline).
...
- `docs/spec_wasi_preview_1.md` — supported subset of `wasi_snapshot_preview1` (MVP set: `proc_exit`, `fd_write`, `fd_read`, `environ_*`, `args_*`, `clock_time_get`, `random_get`, `fd_close`, `fd_seek`, `fd_fdstat_get`), how each WASI import lowers to a synthesised Udon callee, the optional `__udon_meta.wasi` extern overrides, the deferred-stub list returning `errno.nosys`, and the errno value table the translator emits.
